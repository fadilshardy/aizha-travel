    <div class="overflow-hidden transform bg-white rounded-lg shadow hover:-translate-y-1 group">
        <div class="relative flex flex-col mx-auto space-y-1 ">
            <div class="w-full bg-white h-28">
                <img src="{{$destination_card->getThumbnailUrl()}}" alt="tailwind logo" class="object-cover w-full h-full " />
            </div>
                ...
                <div class="flex items-end justify-between gap-x-4">
                    <div class="flex items-center ">
                        ...
                        <p class="ml-1 text-sm font-bold text-gray-600 line-clamp-1 ">
                            {{$destination_card->getAvgRating()}}
                            <span class="font-normal text-gray-500 ">({{$destination_card->getTotalReviews()}} reviews)</span>
                        </p>
                    </div>
                    <div class="flex items-center justify-center gap-1">
                        ...
                        <span class="w-full text-sm text-gray-600 line-clamp-1">{{$destination_card->location}}</span>
                    </div>
                </div>
                <h3 class="text-lg font-bold text-gray-800 break-all truncate hover:text-teal-800"><a href="{{route('user.destination.show', $destination_card->slug)}}">{{$destination_card->name}}</a></h3>
                <p class="text-sm text-gray-500 line-clamp-1">{{$destination_card->summary}}</p>
                <div class="flex items-end justify-between">
                    <div class="text-xs font-bold text-gray-800 xl:text-base">
                        ${{$destination_card->price}}
